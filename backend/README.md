...
多租户 AIOps SaaS 的后端骨架。技术栈：FastAPI + SQLModel + Pydantic AI + Alembic + Celery。
...
> dev 默认使用 `backend/dev.db`（SQLite），零配置启动，无需额外服务。
...
- `app/api/`: HTTP 路由与异常映射
- `app/services/`: 业务编排层
- `app/repositories/`: 数据访问层
- `app/models/`: SQLModel 实体
- `app/schemas/`: API / service 输入输出模型
- `app/agent/`: 诊断与审批工作流运行时
- `app/workers/`: Celery 入口

更细的约束见 [ARCHITECTURE.md](./ARCHITECTURE.md)。
